...
class MNIST_LSTM(nn.Module):
    ...
    def classify(self, features):
        ...
        return self.home_lstm.classify(features)

    def get_classifier_network(self):
        return self.home_lstm.classifier
...
class EEGNet(nn.Module):
    ...
    def classify(self, features):
        features = features.reshape(features.shape[0], 16, 1, 23)
        output = self.classifier(features)
        output = torch.flatten(output, start_dim=1).unsqueeze(1)
        return output
...
class CRNN(nn.Module):
    ...
    def classify(self, features):
        return self.attn_lstm.classify(features)

    def get_classifier_network(self):
        return self.attn_lstm.classifier
